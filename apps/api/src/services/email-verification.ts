...
export function extractEmailVerificationPrefix(rawToken: string): string | null {
...
export async function issueEmailVerificationToken(
  ...
  const raw = `tdls_verify_${tokenPrefix}_${tokenSecret}`;

  return {
    id: newId('evt_'),
    raw,
    tokenPrefix: `tdls_verify_${tokenPrefix}`,
    tokenHash: await hashApiKey(raw, env.API_KEY_PEPPER),
    expiresAt: options?.expiresAt ?? new Date(Date.now() + EMAIL_VERIFICATION_TTL_MS).toISOString(),
  };
}
